        ...
        <a-modal title="新增" 
            ...
            <a-form-model :model="addForm" :label-col="labelCol" :wrapper-col="wrapperCol">
                <a-form-model-item label="ID">
                    <a-input v-model="addForm.id" autocomplete="off" placeholder="自动生成ID，无需填写" :disabled="true"></a-input>
                </a-form-model-item>
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <a-button type="primary" @click="addSysDept">确 定</a-button>
            </div>
        </a-modal>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        handleChange(value) {
            let that = this
            if (value) {
                that.deptId = value[value.length - 1]
            }
        },
        addSysDept: function(){
            let that = this
            if(that.deptIds && that.deptIds.length > 0){
                that.addForm.parentId = that.deptIds[0]
            }
            // 定义请求参数
